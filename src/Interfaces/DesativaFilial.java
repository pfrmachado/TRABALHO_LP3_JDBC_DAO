...
import static Interfaces.TransporteQuantidade.produtoSelecionado;
...
public class DesativaFilial extends javax.swing.JFrame {
    ...
    static EstoqueDAO estoqueDAO;
    static String produtoSelecionado = "";
    static String filialSelecionada = "";


    /**
     * Creates new form DesativaFilial
     */
    public DesativaFilial() throws Exception {
        initComponents();
    ...
    private void btnTransportarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTransportarActionPerformed
        ...
        try
        {
            List<Estoque> estoques = estoqueDAO.buscaByFilial(filialSelecionada);
            String filialDestino = (String)cbbListaFiliais.getSelectedItem();
            if(filialDestino.length() > 0)
            {
                for(Estoque e : estoques)
                {
                    Estoque newE = estoqueDAO.buscaByProdutoByFilial(e.getProduto(), filialDestino);
                    if(newE != null)
                    {
                        newE.setQuantidade(newE.getQuantidade() + e.getQuantidade());
                        newE.setFilial(filialDestino);
                        estoqueDAO.atualizaEstoque(newE, newE.getFilial(), newE.getProduto());
                        estoqueDAO.excluiByProdutoByFilial(e.getProduto(), e.getFilial());
                    }
                    else
                    {
                        String oldFilial = e.getFilial();
                        e.setFilial(filialDestino);
                        estoqueDAO.atualizaEstoque(e, oldFilial, e.getProduto());
                    }
                }
    ...
    public static void main(String args[]) throws Exception {
        ...
        //</editor-fold>
        produtoSelecionado = args[0];
        filialSelecionada = args[1];
        estoque = new Estoque();
        estoqueDAO = new EstoqueDAO();
        estoque = estoqueDAO.buscaByProdutoByFilial(produtoSelecionado, filialSelecionada);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ...
                    Logger.getLogger(DesativaFilial.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
    }
    ...
    public void preencheComboBoxFiliais()
    {
        ...
        try {
            List<Estoque> lista = estoqueDAO.buscaTodasFiliais();
            if(lista.size() > 0)
            {
                for (Estoque f : lista)
                {
                    if(!f.getFilial().equals(estoque.getFilial()))
                        cbbListaFiliais.addItem(f.getFilial());
                }
            }
            else
            {
